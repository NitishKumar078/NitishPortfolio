...
const ENTRIES = [
  {
    ...
    body: (
      <>
        Building a completely new version of my portfolio with{" "}
        <strong>Bebas Neue + DM Mono</strong>, a full responsive system using{" "}
        <code>clamp()</code> throughout, and now this living log feed so
        visitors can see what I&apos;m actually working on day-to-day.
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        I needed reliable favicon fetching for a side project and every solution
        I found either ran in the browser or had heavy dependencies. Wrote a
        clean <strong>Node.js-only utility</strong>, published it on npm, and
        documented it properly. Here&apos;s what I learned about dual ESM/CJS
        exports.
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        Setting up dual <code>exports</code> in <code>package.json</code> sounds
        simple until you hit the edge cases — <code>require()</code> vs{" "}
        <code>import</code>, <code>.mjs</code> extensions, and bundler quirks.
        Spent two hours on something that should take ten minutes. Documenting
        this so future-me doesn&apos;t repeat it.
      </>
    ),
    tags: ["esm", "node", "dx"],
    link: null,
  },
  {
    ...
    body: (
      <>
        Experimenting with a <strong>Manifest V3</strong> Chrome extension using
        React + Vite. The goal: a lightweight tab manager that surfaces context
        without getting in the way. Early stage — just getting the background
        service worker pattern right.
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        Rewrote the keyboard and mouse hook logic in <strong>Spy</strong> to use
        lower-level Win32 hooks instead of WPF event routing. Much more reliable
        across different window types, and no longer drops events under
  ...
  },
  {
    ...
    body: (
      <>
        The more I practice DSA the more I see it show up in real code —
        choosing the right data structure for a cache, recognising a graph
        problem in a dependency resolver, knowing when <code>O(n log n)</code>{" "}
        actually matters. It&apos;s worth doing consistently, not just before
        interviews.
      </>
    ),
    tags: ["dsa", "learning"],
    ...
    <>
      <div className="log-header">
        <div>
          ...
          <h2 className="sec-h rv" id="log-heading" style={{ marginBottom: 0 }}>
            Activity feed.
          </h2>
        </div>
        <div
          className="log-filters rv2"
          role="group"
